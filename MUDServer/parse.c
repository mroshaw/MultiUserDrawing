...
void parseScript(char *inString, char *resultString, ServerDetails *serverDetails) {
    ...
    */
    ...
    sscanf(inString, "%i %s %s %s %i %i %i %i %i %s", &clientId, command, target, &arg1, &arg2, &arg3, &arg4, &arg5,
           &arg6, theText);
...
void testParseScript(char *theString, ServerDetails *serverDetails) {
    char result[255];
    parseScript(theString, result, serverDetails);
    printf("Result: %s\n", result);
}
...
void testParse(ServerDetails *serverDetails) {
    ...
    while (strcmp(string, "EXIT\n") != 0) {
        printf("& ");
        getLine(string, 255);
        testParseScript(string, serverDetails);
    }
}

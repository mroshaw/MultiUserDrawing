...
/* Do the server loop	*/
void runServer(ServerDetails *serverDetails) {
    int port;
    int their_namelen;
    int reclen;
    int them;
    ...
    if (our_socket == INVALID_SOCKET) {
        fprintf(stderr, "Failed to create socket (Error %p).\n", WSAGetLastError);
        exit(1);
    }
    ...
    fprintf(stdout, "Socket successfully created and bound.\n");
    ...
    */
    ...
    while (1) {
        ...
        if ((them = accept(our_socket, (struct sockaddr *) &their_name, &their_namelen)) >= 0) {
            ...
            parseScript(buffer, result, serverDetails);
...
/* Clean up nicely	*/
void die(int value) {
    (void) fprintf(stderr, "Server shutting down.\n");
